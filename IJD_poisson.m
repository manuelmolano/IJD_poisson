function [info_derivative,info,threshold_info,threshold_derivative, H, HS] =...
    IJD_poisson(responses,stimulus,num_sh,jitt_mat,num_bins,span,pintar,results_folder,name,max_num_spikes)
...
for ind_st=1:numel(stimuli)
    ...
    patterns(ind_st,:) = smooth(patterns(ind_st,:),span);
...
%these vector keep the info for the different jitter values
...
for ind_jitt=1:numel(jitt_mat)
    ...
    num_subplots = 16;
    if  mod(ind_jitt-1,ceil(numel(jitt_mat)/num_subplots))==0 && pintar %
...
for ind_sh=1:num_sh
    ...
    for ind_st=1:numel(stimuli)
        ...
        patterns_sh(ind_st,:) = smooth(patterns_sh(ind_st,:),span);
    end
    for ind_jitt=1:numel(jitt_mat)
        %this is the actual jitter
        jitter = jitt_mat(ind_jitt);
...
save([results_folder name 'infoMax'],'info_derivative','info_derivative_sh','info','info_max','info_sh','info_max_sh','jitt_mat','patterns','I_rate')
if pintar
